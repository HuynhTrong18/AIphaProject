...
import { BaseButton,CommandBarButton, IconButton, Dialog, DialogType, Stack } from "@fluentui/react";
...
import { MicButton } from "../../components/common";
...
const Chat = () => {
    ...
    // const [videoAvatar, setVideoAvatar] = useState<string | null>()
    ...
    // const makeApiRequestAvatar = async (message: ChatMessage) => {
    //     const abortController = new AbortController()
    //     const response = await avatarApi(message, abortController.signal);
    //     if (response.ok) {
    //         const data = await response.json() as AvatarResponse;
    //         if (data.outputs.result) {
    //             const videoBlob = await fetch(data.outputs.result).then(res => res.blob());
    //             const videoUrl = URL.createObjectURL(videoBlob);
    //             message.blobURL = videoUrl
    //             setVideoAvatar(null);
    //             setVideoAvatar(videoUrl);
    //         }
    //     }
    //     return message
    // }
    ...
    const makeApiRequestWithoutCosmosDB = async (question: string, conversationId?: string) => {
        ...
        try {
            ...
            if (response?.body && response.ok) {
                ...
                // assistantMessage = await makeApiRequestAvatar(assistantMessage);
                const client_id = appStateContext?.state.client_id ? appStateContext?.state.client_id : uuid()
                if(remoteVideoRef.current){
                    remoteVideoRef.current.muted = false
                    await PostSpeak(assistantMessage.content,client_id)
    ...
    const setupWebRTC = async () => {
        const iceToken = await getIceToken()
        ...
        peerConnection.current.ontrack = (event) => {
            ...
            if (event.track.kind === 'video') {
                if (remoteVideoRef.current) {
                    remoteVideoRef.current.id = event.track.kind
                ...
                if (remoteAudioRef.current) {
                    remoteAudioRef.current.id = event.track.kind
                    remoteAudioRef.current.srcObject = event.streams[0];
                    remoteAudioRef.current.autoplay = false
        ...
        const c = peerConnection.current.createDataChannel("eventChannel")
        ...
        peerConnection.current.onicecandidate = (event) => {
            if (!event.candidate && !iceGatheringDone) {
                ...
                connectToAvatarService(peerConnection.current)
            }
        }

        peerConnection.current.createOffer().then(sdp => {
            if(peerConnection.current) {
                peerConnection.current.setLocalDescription(sdp).then(() => { setTimeout(() => {
                    if (!iceGatheringDone) {
                        iceGatheringDone = true
                        connectToAvatarService(peerConnection.current)
                    }
    ...
    const connectToAvatarService = async (peerConnection: RTCPeerConnection | null) => {
        let localSdp = btoa(JSON.stringify(peerConnection?.localDescription))
        const client_id = appStateContext?.state.client_id ? appStateContext?.state.client_id : uuid()
        const response = await connectAvatar(localSdp, client_id);
    ...
    const handleTranscript = (newTranscript: string) => {
        if(appStateContext?.state.isCosmosDBAvailable?.cosmosDB)
            makeApiRequestWithoutCosmosDB(newTranscript,appStateContext?.state.currentChat?.id ? appStateContext?.state.currentChat?.id : undefined)
        else
            makeApiRequestWithCosmosDB(newTranscript,appStateContext?.state.currentChat?.id ? appStateContext?.state.currentChat?.id : undefined)
    };

    const handelReplayVoice = (mediaStream: MediaStream | undefined) => {
        if(localVideoRef.current && remoteVideoRef.current){
            ...
            if(mediaStream)
                ...
                localVideoRef.current.src = "./avatarDefault.mp4"
        }
    };

    const handelEndLocalVideo = () => {
        if(localVideoRef.current && remoteVideoRef.current){
    ...
    return (
        <div className={styles.chatRoot} role="main">
            {showAuthMessage ? (
            ...
            <>
                <Stack horizontal className={styles.avatarcontainer}>
                    <video className={styles.videosection} ref={remoteVideoRef}/>
                    <audio ref={remoteAudioRef}/>
                    <video className={styles.videosection} ref={localVideoRef} playsInline hidden onEnded={handelEndLocalVideo}/>
                                    ...
                                    <>
                                        {answer.role === "user" ? (
                                            ...
                                            <div className={styles.chatMessageGpt}>
                                                ...
                                                />
                                                <IconButton iconProps={{ iconName: 'Volume3'}} aria-label="replay voice" onClick={()=>handelReplayVoice(answer.mediaStream)}/>
